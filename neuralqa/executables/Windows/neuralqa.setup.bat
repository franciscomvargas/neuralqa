...
:: USER PATH
:: %~dp0 = C:\Users\[username]Desota\Desota_Models\NeuralQA\neuralqa\executables\windows
for %%a in ("%~dp0\..\..\..\..\..\..") do set "user_path=%%~fa"
for %%a in ("%~dp0\..\..\..\..\..\..\..") do set "test_path=%%~fa"
for %%a in ("%UserProfile%\..") do set "test1_path=%%~fa"

:: Model VARS
set model_name=NeuralQA
set model_path_in_basepath=Desota\Desota_Models\%model_name%
set model_path_basepath=%model_path_in_basepath%\neuralqa

:: - Model GIT
set model_release=https://github.com/franciscomvargas/neuralqa/archive/refs/tags/v0.0.0.zip
...
set pip_reqs_basepath=%model_path_in_basepath%\requirements.txt

:: - Service as NSSM VARS
set model_service_basepath=%model_path_basepath%\executables\Windows
set model_service_install_basepath=%model_service_basepath%\neuralqa.nssm.bat
set model_start_basepath=%model_service_basepath%\neuralqa.start.bat
...
:TEST_PASSED
:: Model VARS
set model_path_in=%user_path%\%model_path_in_basepath%
...
IF NOT EXIST %model_path_in% (
    ...
    ECHO     IF EXIST %model_path_in% ^( rmdir /S /Q %model_path_in% ^) ELSE ^( ECHO New Install! ^) ^&^& mkdir %model_path_in% ^&^& powershell -command "Invoke-WebRequest -Uri %model_release% -OutFile %user_path%\%model_name%_release.zip" ^&^&  tar -xzvf %user_path%\%model_name%_release.zip -C %model_path_in% --strip-components 1 ^&^& del %user_path%\%model_name%_release.zip
    ECHO  %ESC%P
    PAUSE
    exit
)
